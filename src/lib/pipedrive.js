...
/**
 * GET request a API v2 (header x-api-token)
 */
...
/**
 * GET request a API v1 (query api_token) – para endpoints que solo existen en v1
 */
...
/**
 * Actividades pendientes o atrasadas (no completadas con due_date <= hoy o sin fecha).
 * API v2: GET /api/v2/activities con done=false.
 */
...
/**
 * Obtener todas las actividades no completadas (paginação si hace falta)
 */
async function getAllActivitiesNotDone(options = {}) {
  ...
  do {
    const result = await getActivitiesPendingAndOverdue({
      ownerId,
      limit,
      cursor,
    })
...
/**
 * Tipos de actividad (v1) para mapear key -> nombre
 */
...
/**
 * Detalle de una actividad por ID (v2)
 */
...
/**
 * Actividades anteriores de un deal/person (completadas) para contexto
 */
...
/**
 * Marcar actividad como completada (v2)
 */
...
/**
 * Crear nueva actividad (v2). due_date en formato YYYY-MM-DD.
 */
...
/**
 * Personas de una organización (v1) – para checklist de destinatarios
 */
...
/**
 * Email principal de una persona (compat con formato Pipedrive)
 */
