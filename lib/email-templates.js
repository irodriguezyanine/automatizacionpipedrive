...
function getSignatureAssetBase() {
  const raw =
    process.env.SIGNATURE_ASSET_BASE_URL ||
    process.env.NEXT_PUBLIC_APP_URL ||
    (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : '') ||
    'https://automatizacionpipedrive.vercel.app'
  return String(raw).replace(/\/$/, '')
}

function assetUrl(path) {
  return `${getSignatureAssetBase()}${path.startsWith('/') ? path : `/${path}`}`
}
...
/** Firma HTML Vedisa (layout igual al diseño corporativo). */
function getSignatureHtml() {
  const logoUrl = assetUrl('/email/vedisa-logo-bloque.png')
  const waIconUrl = assetUrl('/email/icon-whatsapp.png')
  const igIconUrl = assetUrl('/email/icon-instagram.png')
  const fbIconUrl = assetUrl('/email/icon-facebook.png')
  const liIconUrl = assetUrl('/email/icon-linkedin.png')
