...
async function searchOrganizations(term, options = {}) {
  ...
  const pageSize = Math.min(500, maxTotal)
  const byId = new Map()
  let start = 0
  for (let guard = 0; guard < 20 && byId.size < maxTotal; guard++) {
    const json = await getV1('organizations', {
      term: raw,
      start,
      limit: Math.min(pageSize, maxTotal - byId.size),
    })
    const data = json.data || []
    for (const o of data) {
      if (o?.id != null && !byId.has(o.id)) {
        byId.set(o.id, { id: o.id, name: o.name || `Organización #${o.id}` })
      }
    }
    const pag = json.additional_data?.pagination
    if (!pag?.more_items_in_collection || data.length === 0) break
